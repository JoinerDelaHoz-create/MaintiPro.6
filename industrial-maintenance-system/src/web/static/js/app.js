...
async function loadEquipmentKPI(code) {
  ...
  try {
    ...
    const k = res.kpi;
    renderEquipmentKPICard(container, k);
  } catch (err) {
    // Fallback: calculate KPIs locally using demo/cached data
    const eq = state.equipments.find((e) => e.code === code);
    const allWOs = state.workOrders.length > 0 ? state.workOrders : getLocalOrDemoWorkOrders();
    ...
    if (eq) {
      ...
      container.innerHTML = `<div class="empty-state-sm text-red">No se pudo cargar la ficha del activo '${code}'. Verifique que el servidor esté activo.</div>`;
    }
  }
}
